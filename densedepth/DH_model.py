...
class Encoder(nn.Module):
    def __init__(self, encoder_pretrained=True):
        ...
        self.densenet = models.densenet169(pretrained=encoder_pretrained)
...
class Decoder(nn.Module):
    def __init__(self, num_features=1664, decoder_width=0.5, scales=[1, 2, 4, 8]):
        ...
        self.upsample1 = Upsample(
            features // scales[0] + 256, features // (scales[0] * 2)
        )
        self.upsample2 = Upsample(
            features // scales[1] + 128, features // (scales[1] * 2)
        )
        self.upsample3 = Upsample(
            features // scales[2] + 64, features // (scales[2] * 2)
        )
        self.upsample4 = Upsample(
            features // scales[3] + 64, features // (scales[3] * 2)
        )
